 ...
 * Input should be Array of 3 sensor message with 'value', 'lat' and 'lon'
 * Output maybe an Array
 */
const getSolution = async (sensors) => {
  const equation = sensors.map((sensor) => `${sensor.value} = k + 10 log((${sensor.lat}-a)^2+(${sensor.lon}-b)^2)`).join(' ; ');
